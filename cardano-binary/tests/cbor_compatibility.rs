...
use cardano_binary::{decode_full, decode_nested_cbor, encode_nested_cbor, serialize};
use serde::de::DeserializeOwned;
use serde::{Deserialize, Serialize};

fn encode_value<T: Serialize>(value: &T) -> Vec<u8> {
    serialize(value).expect("serialization must succeed for compatibility test")
}

#[allow(clippy::missing_panics_doc)]
fn decode_value<T: DeserializeOwned>(bytes: &[u8]) -> T {
    decode_full(bytes).expect("deserialization must succeed for compatibility test")
}

fn encode_nested_value<T: Serialize>(value: &T) -> Vec<u8> {
    encode_nested_cbor(value).expect("nested CBOR encoding must succeed for compatibility test")
}

fn decode_nested_value<T: DeserializeOwned>(bytes: &[u8]) -> T {
    decode_nested_cbor(bytes).expect("nested CBOR decoding must succeed for compatibility test")
}
...
fn cbor_compat_unsigned_integers() {
    ...
    assert_eq!(encode_value(&0u8), vec![0x00]);
    assert_eq!(encode_value(&1u8), vec![0x01]);
    assert_eq!(encode_value(&10u8), vec![0x0a]);
    assert_eq!(encode_value(&23u8), vec![0x17]);

    // Medium integers (24-255) encode as 0x18 + byte
    assert_eq!(encode_value(&24u8), vec![0x18, 0x18]);
    assert_eq!(encode_value(&42u8), vec![0x18, 0x2a]);
    assert_eq!(encode_value(&255u8), vec![0x18, 0xff]);

    // Larger integers
    assert_eq!(encode_value(&256u16), vec![0x19, 0x01, 0x00]);
    assert_eq!(encode_value(&1000u16), vec![0x19, 0x03, 0xe8]);

    // u32 values
    assert_eq!(encode_value(&100000u32), vec![0x1a, 0x00, 0x01, 0x86, 0xa0]);

    // u64 values
    assert_eq!(
        encode_value(&0x0102030405060708u64),
...
fn cbor_compat_negative_integers() {
    ...
    assert_eq!(encode_value(&-1i8), vec![0x20]);

    // -10 encodes as 0x29 (negative 9)
    assert_eq!(encode_value(&-10i8), vec![0x29]);

    // -24 encodes as 0x37 (negative 23)
    assert_eq!(encode_value(&-24i8), vec![0x37]);

    // -25 encodes as 0x38 0x18 (negative 24)
    assert_eq!(encode_value(&-25i8), vec![0x38, 0x18]);

    // -100 encodes as 0x38 0x63 (negative 99)
    assert_eq!(encode_value(&-100i16), vec![0x38, 0x63]);

    // -1000 encodes as 0x39 0x03 0xe7 (negative 999)
    assert_eq!(encode_value(&-1000i16), vec![0x39, 0x03, 0xe7]);
}

/// Test boolean encoding
#[test]
fn cbor_compat_booleans() {
    assert_eq!(encode_value(&false), vec![0xf4]);
    assert_eq!(encode_value(&true), vec![0xf5]);
}
...
fn cbor_compat_null() {
    ...
    assert_eq!(encode_value(&value), vec![0xf6]);
}
...
fn cbor_compat_strings() {
    // Empty string
    assert_eq!(encode_value(&""), vec![0x60]);

    // "a" - single character
    assert_eq!(encode_value(&"a"), vec![0x61, 0x61]);

    // "IETF" - 4 characters
    assert_eq!(encode_value(&"IETF"), vec![0x64, 0x49, 0x45, 0x54, 0x46]);
    ...
    let water = "水";
    let encoded = encode_value(&water);
...
fn cbor_compat_arrays() {
    ...
    assert_eq!(encode_value(&empty), vec![0x80]);

    // [1, 2, 3]
    let arr = vec![1u32, 2, 3];
    assert_eq!(encode_value(&arr), vec![0x83, 0x01, 0x02, 0x03]);

    // [1, [2, 3], [4, 5]]
    let nested = vec![vec![1u32], vec![2, 3], vec![4, 5]];
    let encoded = encode_value(&nested);
    assert_eq!(
        encoded,
        vec![
            0x83, // array(3)
...
fn cbor_compat_structs() {
    ...
    let encoded = encode_value(&point);

    // Should be a map with 2 entries
    assert_eq!(encoded[0], 0xa2); // map(2)

    // Verify roundtrip
    let decoded: Point = decode_value(&encoded);
...
fn cbor_compat_tuples() {
    ...
    assert_eq!(encode_value(&tuple), vec![0x83, 0x01, 0x02, 0x03]);
...
fn cbor_compat_options() {
    ...
    assert_eq!(encode_value(&none), vec![0xf6]);

    // Some(42) -> just the value
    let some: Option<u32> = Some(42);
    assert_eq!(encode_value(&some), vec![0x18, 0x2a]);

    // Some("test") -> just the string
    let some_str: Option<&str> = Some("test");
    assert_eq!(encode_value(&some_str), vec![0x64, 0x74, 0x65, 0x73, 0x74]);
}
...
fn cbor_compat_tags() {
    ...
    let encoded = encode_value(&tagged);
...
fn cbor_compat_cardano_like_struct() {
    ...
    let encoded = encode_value(&tx);

    // Should be a map with 3 keys
    assert_eq!(encoded[0], 0xa3); // map(3)

    // Verify roundtrip
    let decoded: Transaction = decode_value(&encoded);
...
fn cbor_compat_canonical_maps() {
    ...
    let decoded: MultiField = decode_value(&encoded);
...
fn cbor_compat_serde_bytes() {
    ...
    let vec_encoded = encode_value(&vec);
    ...
    // Verify roundtrip
    let decoded: ByteBuf = decode_value(&encoded);
...
fn cbor_compat_nested_cbor() {
    ...
    let nested = encode_nested_value(&inner);
    ...
    // Verify roundtrip
    let decoded: ByteBuf = decode_nested_value(&nested);
...
fn cbor_compat_cardano_examples() {
    ...
    let decoded: Address = decode_value(&encoded);
    ...
    let encoded = encode_value(&addr_no_stake);
    let decoded: Address = decode_value(&encoded);
...
fn cbor_compat_deterministic() {
    ...
    let encoded1 = encode_value(&data);
    let encoded2 = encode_value(&data);
    let encoded3 = encode_value(&data);
...
fn cbor_compat_max_values() {
    ...
    // Max u16
    assert_eq!(encode_value(&65535u16), vec![0x19, 0xff, 0xff]);

    // Max u32
    assert_eq!(encode_value(&u32::MAX), vec![0x1a, 0xff, 0xff, 0xff, 0xff]);

    // Large u64
    assert_eq!(
        encode_value(&u64::MAX),
