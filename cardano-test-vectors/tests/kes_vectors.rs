...
fn single_kes_vectors_reject_tampered_messages() {
    ...
    let vector = parsed
        .vectors
        .first()
        .expect("at least one SingleKES vector present");

    let seed_bytes = decode_hex(&vector.seed);
    let message = decode_hex(&vector.message);
    let tampered = tamper_message(&message);

    let signing_key = SingleKes::<Ed25519>::gen_key_kes_from_seed_bytes(&seed_bytes)
        .expect("single signing key generation");
    let verification_key = SingleKes::<Ed25519>::derive_verification_key(&signing_key)
        .expect("single verification key derivation");

    let raw_signature_bytes = decode_hex(&vector.expected.raw_signature);
    let signature = SingleKes::<Ed25519>::raw_deserialize_signature_kes(&raw_signature_bytes)
        .expect("single signature deserialise");

    let err = SingleKes::<Ed25519>::verify_kes(
        &(),
        &verification_key,
        vector.period,
        &tampered,
...
fn compact_single_kes_vectors_reject_tampered_messages() {
    ...
    let vector = parsed
        .vectors
        .first()
        .expect("at least one CompactSingleKES vector present");
    ...
    let signing_key = CompactSingleKes::<Ed25519>::gen_key_kes_from_seed_bytes(&seed_bytes)
        .expect("compact single signing key generation");
    let verification_key = CompactSingleKes::<Ed25519>::derive_verification_key(&signing_key)
    ...
    let err = CompactSingleKes::<Ed25519>::verify_kes(
        &(),
        &verification_key,
        vector.period,
        &tampered,
...
fn sum_kes_vectors_reject_tampered_messages() {
    ...
    for level in &parsed.levels {
        ...
        match level.level {
            ...
            3 => assert_sum_tampered_message_fails::<Sum3Kes>(level),
            4 => assert_sum_tampered_message_fails::<Sum4Kes>(level),
...
fn compact_sum_kes_vectors_reject_tampered_messages() {
    ...
    for level in &parsed.levels {
        match level.level {
            1 => assert_sum_tampered_message_fails::<CompactSum1Kes>(level),
            2 => assert_sum_tampered_message_fails::<CompactSum2Kes>(level),
            3 => assert_sum_tampered_message_fails::<CompactSum3Kes>(level),
            4 => assert_sum_tampered_message_fails::<CompactSum4Kes>(level),
    ...
    K: KesAlgorithm<Context = ()>,
{
    let vector = level
        .vectors
        .first()
        .expect("at least one tracked vector present for level");
    let period = vector
        .tracked_periods
        .first()
        .expect("tracked periods available for tamper test");

    let seed_bytes = decode_hex(&vector.seed);
    let signing_key = K::gen_key_kes_from_seed_bytes(&seed_bytes).expect("tamper signing key");
    let verification_key =
        K::derive_verification_key(&signing_key).expect("tamper verification key derivation");

    let message = decode_hex(&period.message);
    let tampered = tamper_message(&message);
    let signature_bytes = decode_hex(&period.raw_signature);
    let signature =
        K::raw_deserialize_signature_kes(&signature_bytes).expect("tamper signature deserialise");

    let err = K::verify_kes(&(), &verification_key, period.period, &tampered, &signature)
        .expect_err("tampered message must fail verification");
    assert!(matches!(err, KesError::VerificationFailed));
...
fn tamper_message(message: &[u8]) -> Vec<u8> {
